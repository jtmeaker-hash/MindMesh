...
export const RoutineModule: React.FC<RoutineModuleProps> = ({ categories, contacts = [], reminders = [], directDebits = [], onRoutinesChange, appearance = getDefaultAppearance() }) => {
  ...
  return <div className="mm-module routine-module" style={{ flex: '1 1 0%', minHeight: 0, overflow: 'auto', width: '100%', minWidth: 0, boxSizing: 'border-box', padding: '24px', color: '#e2e8f0', background: '#090d16' }}>
    <div style={{ maxWidth: 1400, minWidth: 0, margin: '0 auto' }}>
      ...
      <div className="routine-builder-layout" style={{ display: 'grid', gridTemplateColumns: selected ? 'minmax(0, .75fr) minmax(0, 1.5fr)' : 'minmax(0, 1fr)', gap: 14, alignItems: 'start', minWidth: 0, maxWidth: '100%' }}>
        ...
        {selected && <section data-testid="routine-builder" data-editor-mode={view} style={{ ...surface, overflow: 'hidden' }}><div style={sectionHeader}><div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><strong>Builder</strong><span style={{ color: saved ? '#34d399' : '#fbbf24', fontSize: 12, display: 'flex', alignItems: 'center', gap: 4 }}>{saved ? <Check size={13} /> : <Save size={13} />} {saved ? 'Saved' : 'Unsaved changes'}</span></div><div style={{ display: 'flex', gap: 5 }}><button type="button" onClick={undo} disabled={!history.length} style={iconButton}><Undo2 size={15} /></button><button type="button" onClick={redo} disabled={!future.length} style={iconButton}><Redo2 size={15} /></button><button type="button" onClick={() => setSelectedId(null)} style={iconButton}><X size={15} /></button></div></div>
          <div style={{ padding: 18, minWidth: 0 }}><div role="tablist" aria-label="Routine editor mode" style={{ display: 'flex', gap: 6, marginBottom: 14, flexWrap: 'wrap' }}><button type="button" role="tab" aria-selected={view === 'list'} onClick={() => setView('list')} style={toggle(view === 'list')}><List size={15} /> List editor</button><button type="button" role="tab" aria-selected={view === 'graph'} onClick={() => setView('graph')} style={toggle(view === 'graph')}><Grid3X3 size={15} /> Node graph</button></div>
            {view === 'list' ? <div data-testid="routine-list-editor">
            ...
            <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap', marginBottom: 14 }}><button type="button" onClick={() => { const template = saveRoutineAsTemplate(selected); window.alert(`Saved “${template.name}” as a reusable template.`); }} style={button('#334155')}>Save as template</button><button type="button" onClick={() => downloadText(`${selected.name || 'routine'}.json`, serializeRoutineExport(selected))} style={button('#334155')}>Export JSON</button><button type="button" onClick={() => downloadText(`${selected.name || 'routine'}-history.json`, serializeRoutineHistoryJson(selected))} style={button('#334155')}>History JSON</button><button type="button" onClick={() => downloadText(`${selected.name || 'routine'}-history.csv`, serializeRoutineHistoryCsv(selected), 'text/csv')} style={button('#334155')}>History CSV</button><input aria-label="Routine helper prompt" value={aiPrompt} onChange={(e) => setAiPrompt(e.target.value)} placeholder="Describe steps to suggest…" style={{ ...input, flex: '1 1 220px' }} /><button type="button" disabled={aiBusy || !aiPrompt.trim()} onClick={async () => { setAiBusy(true); setAiProposal(await createRoutineAiProposal('suggest', aiPrompt, selected)); setAiBusy(false); }} style={button('#155e75')}>{aiBusy ? 'Preparing local draft…' : 'Suggest routine steps'}</button></div>
            {aiProposal && <div role="dialog" aria-label="Routine proposal" style={{ ...surface, padding: 12, marginBottom: 14, borderColor: '#22d3ee' }}><strong>Local proposal — review before applying</strong>{aiProposal.changes.map((change) => <label key={change.id} style={{ display: 'flex', gap: 8, padding: '7px 0', color: '#cbd5e1', fontSize: 12 }}><input type="checkbox" defaultChecked value={change.id} /> <span>{change.description}</span></label>)}<div style={{ display: 'flex', gap: 7, marginTop: 8 }}><button type="button" onClick={() => { const ids = Array.from(document.querySelectorAll('[aria-label="Routine proposal"] input:checked')).map((node) => (node as HTMLInputElement).value); const next = applyApprovedAiChanges(selected, aiProposal, ids); persist(next); setAiProposal(null); }} style={button('#0f766e')}>Apply selected</button><button type="button" onClick={() => setAiProposal(null)} style={button('#334155')}>Discard</button></div></div>}
...
const GraphView: React.FC<{ routine: Routine; appearance: AppearanceSettings; onChange: (id: string, patch: Partial<RoutineStep>) => void }> = ({ routine, appearance, onChange }) => {
  ...
  return <div data-testid="routine-graph-viewport" className="routine-graph-viewport" style={{ position: 'relative', width: '100%', maxWidth: '100%', minWidth: 0, minHeight: 320, height: 'min(62vh, 520px)', overflow: 'hidden', borderRadius: 12, background: 'radial-gradient(circle at 50% 20%, rgba(8,145,178,.14), transparent 55%), #0b1220', border: '1px solid #1e3a4a' }}><div data-testid="routine-graph" style={{ width: '100%', maxWidth: '100%', minWidth: 0, height: '100%', minHeight: 0, borderRadius: 12, overflow: 'hidden' }}>
